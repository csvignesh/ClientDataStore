...
describe('data-store/local-storage/remove', function() {
    ...
    it('removes the value based on one key in an index', function() {
        var database = null;
        return lStorage.init('db1-remove', [
            {
                indexes: [{name: 'attr1'}],
                name: 'table1'
            }
    ...
    it('removes the value based on an array of keys in an index', function() {
        ...
        return lStorage.init('db2-remove', [
            ...
            return lStorage.destroy('db2-remove');
        });
    });

    it('removes all data if there is no filter', function() {
        var database = null;
        return lStorage.init('db3-remove', [
            {
                indexes: [{name: 'attr1'}],
                name: 'table1'
            }
    ...
    it('throws an error if an remove is attempted on a non existing table', function(done) {
        ...
        return lStorage.init('db4-remove', [
            ...
            return database.remove({name: 'invalidtable'}, {}).fail(function() {
                return lStorage.destroy('db1-remove').then(function() {
                    done();
                });
            });
        });
    ...
    it('doesn\'t remove anything if key search in index doesn\'t fetch any data', function() {
        var database = null;
        return lStorage.init('db5-remove', [
            {
                indexes: [{name: 'attr1'}],
                name: 'table1'
            }
            ...
            return lStorage.destroy('db5-remove');
        });
    });
});
