...
describe('data-store/local-storage/update', function() {
    ...
    it('throws an error if an update is attempted on a non existing table', function(done) {
        var database = null;
        //to be fixed
        return lStorage.init('db4-update', [
            {
                indexes: [{name: 'attr1'}],
                name: 'table1'
            }
            ...
            return database.update({
                ...
                return lStorage.destroy('db4-update').then(function() {
                    done();
                });
            });
        });
    ...
    it('doesn\'t update anything if key search in index doesn\'t fetch any data', function() {
        ...
        var database = null;
        //to be fixed
        return lStorage.init('db5-update', [
            {
                indexes: [{name: 'attr1'}],
                name: 'table1'
            }
            ...
            expect(data.length).to.eql(0);
            return lStorage.destroy('db5-update');
        });
    });
});
